...
#         had to "conda install hvplot" to allow import
...
#events_paths = [str(p) for p in data_path.glob('*_rawmed.txt')]
events_paths

# %%
events = pd.read_csv(events_paths[0])               # [0] is indexing the first item in events_paths
...
acq_list = [pd.read_csv(path) for path in events_paths]

# Apply pd.to_timedelta on 'onset' column for each DataFrame in acq_list
for df in acq_list:
    df['onset'] = pd.to_timedelta(df['onset'])

# Now each DataFrame in acq_list has the 'onset' column converted to timedelta
